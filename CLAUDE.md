...
**Last Updated:** 2026-06-23 (Session #20 — 3 new to-do items logged: Google Alert, homepage change-detection monitor, tagline/footer rewording)
**Owner:** Felix Okon
...
- [ ] **🔴 NEW — Add `chat:write` scope to `SLACK_BOT_TOKEN` and reinstall the Slack app.** Discovered Session #19: the bot token only has `channels:read`, `channels:history`, `groups:read`, `groups:history` — confirmed via the `x-oauth-scopes` response header on a real `chat.postMessage` call. `chat:write` (Slack reports it as `chat:write:bot`) was never granted. **Every threaded Slack reply this pipeline has ever attempted — `replyToFelix()` since Session #9, and the new `replyFailureToFelix()` from this session — has been silently failing.** Felix has never actually seen a "✅ Used in: ..." thread confirmation; only the separate #aitrends-feed webhook notifications have been arriving, which is why this went unnoticed for 13 sessions. Fix: Felix adds `chat:write` to the Slack app's OAuth scopes in api.slack.com → reinstalls the app → copies the new bot token → updates `SLACK_BOT_TOKEN` in `.env` and GitHub Actions Secrets.
...
- [ ] **Update tagline + footer text (added Session #20, 23 June 2026)** — Felix's exact wording:
  - Tagline (meta description, currently confirmed live): `"Africa's autonomous briefing on AI — the latest news, trends, and what it means for builders in Nigeria and across the continent."` → `"AItrends.ng-- insights, and lessons for the future."`
  - Footer line: `"Today and the next AI trends"` → same new text: `"AItrends.ng-- insights, and lessons for the future."` (confirmed with Felix — footer mirrors the new tagline, not separate wording)
  - Per Session #1/Round 1 history, this tagline/footer string appears in at least: `app/layout.tsx`, `components/Footer.tsx`, `components/Sidebar.tsx`, `app/about/page.tsx` — all 4 locations need updating together, same pattern as the "Today and the next AI trends" rollout in Round 1.
  - Not yet executed — Felix said "add to to-do list," not "do it now."

### 🟡 Medium
- [ ] **Set up a Google Alert for "AITrends.ng" or `site:aitrends.ng` (added Session #20)** — email notification when new content gets indexed by Google. Manual setup at google.com/alerts, no code involved. Complements the still-pending sitemap.xml GSC submission above.
- [ ] **Set up a change-detection monitor on the homepage (added Session #20)** — Felix wants an alert if: (1) the site title or meta description changes, (2) new categories or nav links appear, (3) non-AI keywords show up on the homepage. Two viable approaches, decide at implementation time:
  - Third-party (no code): a service like Visualping or Distill.io watching `https://www.aitrends.ng/` for the relevant DOM regions.
  - Custom-built (consistent with this project's existing pattern of building its own automation rather than relying on third-party tools): a small scheduled script — fetch the homepage, extract `<title>`/meta description/nav `<a>` hrefs, diff against the last-known snapshot stored in Supabase, run a simple keyword check against an AI/Africa vocabulary list (the project already has `evergreen_vocab` and `hasAISignal()` precedent to reuse), and post to Slack on any diff. Would live in `scout-agent/` as a new lightweight script + GHA workflow on its own schedule (e.g. every 6h, independent of Phase 1/2).
