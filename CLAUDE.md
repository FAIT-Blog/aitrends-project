...
**Last Updated:** 2026-08-18 (Session #46 — Groq provider restored: dead `llama-3.3-70b-versatile` (HTTP 404, deprecated 2026-08-16) replaced with `openai/gpt-oss-120b` (120B params, 65K output, 1K RPD, 8K TPM free tier), Tavily-off experiment preserved, Aug 16–18 marked EXPERIMENT INVALID — PROVIDER FAILURE, new experimental baseline established)
**Owner:** Felix Okon
...
scout-agent/
  ...
  registry.seed.json ← The curated source universe (v3: 21 sources) — canonical seed data, supersedes feeds.js as source of truth
...
- **Seed:** `registry.seed.json` v3 = 21 sources. `seed-registry.js` is explicit — read-only diff without
...
- **Governance:** a source is a source because the registry says so, not because code touches it. Stage 1A
  ...
  defunct) and the WeeTracker URL fix, settling the universe at 21 sources (15 enabled, feed-bearing). Session #45
